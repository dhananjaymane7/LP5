...
#define TOLERANCE 1e-3          // Increased tolerance for better precision handling
#define USE_DOUBLE 0            // Set to 1 for double precision, 0 for single precision
...
int main() {
    ...
    int isValid = 1;
    for (int i = 0; i < N * N; i++) {
        if (fabs(h_C[i] - h_C_gpu[i]) > TOLERANCE * fabs(h_C[i])) {  // Relative tolerance check
            isValid = 0;
            break;
        }
    }
